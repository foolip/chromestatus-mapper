...
def export(json_file_path):
    ...
    for entry_id, outcome in data.items():
        match outcome:
            case {"failure": _}:
                failures.append({"entry": entry_id, "notes": outcome.get("notes", "")})
            case {"result": _}:
                ...
                if feature_id in features:
                    ...
                    print(
                        f"Warning: treating a mapping for {entry_id} to {feature_id} as a failure (invalid ID)"
                    )
                    failures.append(
                        {
                            "entry": entry_id,
            ...
            case _:
                print(
                    f"Warning: skipping a mapping that was neither a success nor a failure: {id}"
                )
